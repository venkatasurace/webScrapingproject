...
#step 1 getting website all pages urls in list formate
...
# step 2 with step 1, step 2.1 ,step 2.2
def scrape_quotes_page():
  print("scraping of quotes_page started ")
  quotes_page_urls_list = generate_quotes_page_urls()
  quotes_containers = get_all_quotes_containers(quotes_page_urls_list)#all html content
  quotes_in_all_pages = create_quotes_dict_list(quotes_containers )#all text in list formate
...
def get_authors_details(quotes_container):
  ...
  for each_author_details in quotes_container:
    scraped_author_page_url = each_author_details.find("a")["href"]#author about
    
    author_name = each_author_details.find("small",class_="author").text# author name
...
#step 3.1 with step 3.1.1, step 3.1
...
# step 3 with step 2,step 3.1(because authors details (dob ,place ,ref))
# main function(quotes_in_all_pages,authors_dict)

def scrape_pages():
  print("scraping of pages is started ")
  quotes_containers,quotes_in_all_pages = scrape_quotes_page()#all pages html,all quotes,authors,tags lists
  authors_dict = scrape_authors_page(quotes_containers)# ref 3.1
